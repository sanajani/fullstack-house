  ...
  return (
    ...
    <div className='space-y-4'>
      <div className='flex justify-between items-center py-3 border-b border-blue-500/30 last:border-0'>
        ...
        <span className='font-bold text-xl'>{showSingleProperty.price?.amount.toLocaleString()}</span>
      </div>
      <div className='flex justify-between items-center py-3 border-b border-blue-500/30 last:border-0'>
        <span className='text-blue-100 text-sm flex items-center gap-2'>
          <span className='text-yellow-300'>🏦</span>
      ...
      <div className='flex justify-between items-center py-3'>
        ...
        <span className={`font-bold ${showSingleProperty.price?.negotiable ? 'text-green-300' : 'text-red-300'}`}>
          {showSingleProperty.price?.negotiable ? '✅ بلی' : '❌ نه'}
        </span>
      </div>
    </div>
