...
//   const { data: propertyResponse, isLoading, isError } = useSinglePropertyByAgentById(id);
...
//     defaultValues: {}, // We'll reset once data is fetched
//   });
...
//           totalFloor: property.details?.totalFloor || "",
//           yearBuild: property.details?.yearBuild || "",
...
//       // Prefill Dropzone local state
//    if (dropzoneRef.current && property.media?.length > 0) {
//   dropzoneRef.current.setImages(
//     property.media.map(img => ({
//       ...img,
//       file: null,        // no actual file because it's from server
//       preview: img.url,  // Dropzone uses this to display the image
//       loading: false,    // server images are already loaded
//     }))
//   );
// }
//     }
...
//     const payload = buildPropertyFormData(formData, imagesWithFiles);
...
//         toast.error("خطا در آپدیت ملک");
//       }
//     });
//   };

//   if (isLoading) return <h1 className="text-center mt-10">در حال بارگذاری اطلاعات ملک...</h1>;
//   if (isError) return <h1 className="text-center mt-10 text-red-500">خطا در دریافت اطلاعات ملک</h1>;

//   return (
//     <div className="min-h-screen bg-gray-100 flex justify-center md:p-6">
//       <form className="w-full md:max-w-5xl bg-white shadow-2xl rounded-xl p-3 md:p-6 space-y-4 md:space-y-6"
//             onSubmit={handleSubmit(onSubmit)}>

//         <h1 className="md:text-2xl text-lg font-bold text-white bg-blue-700 p-3 rounded-lg">ویرایش خانه</h1>
...
//         <button className={`w-full bg-blue-700 text-white py-3 rounded-lg font-semibold hover:opacity-90 cursor-pointer ${loading ? "opacity-50 cursor-not-allowed bg-red-600" : ""}`}
//                 disabled={loading} type="submit">
//           {!loading ? "به‌روزرسانی ملک" : "در حال پردازش..."}
//         </button>
//       </form>
//     </div>
//   );
